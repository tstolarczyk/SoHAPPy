...
This modules compute the rise and set, night (twilight) windows, and combine
them to get visibility windows for a given GRB on a given site.
It is used either to compute the visibiliy windows and check that the ones given 
by default are in agreement for the default horizon value (10 degrees).
The windows foudn are passed to the GammaRayBurst class for further processing.
...
class Visibility():
    ...
    being visible.
    One of the method is used to compare the visibility windows with the one
    ...
    ###------------------------------------------------------------------------
    def compute(self,altmin = 10*u.degree, depth=1, end=0, npt=150, debug=False):    
        ...
        depth : int, optional
            ...
            The default is 1, and the reference time is the start of the 
            visibility window (i.e. the window end can be beyon the depth)
        end : integer, optional
            Defines the crietria tpo accept a visibility windo within the 
       ...
       # Temporrary local variables in Julian days (to use sort)
        ...
        t_dawn = obs.twilight_morning_astronomical(t_dusk, 
                                                   which="next", 
                                                   n_grid_points = npt)
        t_night.append([t_dusk.jd, t_dawn.jd])
        
        # Add a second night of within 24h
        t_dusk2 = obs.twilight_evening_astronomical(t_dawn, 
                                                    which="next",  
                                                    n_grid_points = npt)
        t_dawn2 = obs.twilight_morning_astronomical(t_dusk2, 
                                                    which="next", 
                                                    n_grid_points = npt)       
        t_night.append([t_dusk2.jd, t_dawn2.jd])
        ...
        ### HORIZON ---
        # Get period above horizon
        high = obs.target_is_up(self.grb.t_trig, target, horizon = self.altmin)
        ...
        if (math.isnan(t_rise.jd)): 
            ...
            # Add a seocnd period
            t_rise2 = obs.target_rise_time(t_set, 
                                      target, 
                                      which="next", 
                                      horizon = self.altmin, 
                                      n_grid_points = npt) 
            t_set2 = obs.target_set_time(t_rise2, 
                                        target, 
                                        which="next", 
                                        horizon = self.altmin, 
                                        n_grid_points = npt)
            t_above.append([t_rise2.jd,t_set2.jd]) 
        ...
        for elt in t_vis:
            if (elt[end] - self.grb.t_trig.jd < depth):
...
if __name__ == "__main__":
    ...
    check      = True # perform visibility window check within tolerance
    ...
    for igrb in grblist:
        ...
        for loc in sites:
            ...
            vis.compute(altmin=altmin, end=1, debug=debug,npt=npt)
            ...
            if (not matching) or (showplot):
                ...
                print(" ORIGINAL")
                grb.show_visibility(loc=loc,log=log) # original visibility 
                gplt.visibility_plot(grb,ax=ax1,loc=loc) 
                
                print(" NEW !!!")
                grb.show_visibility(loc=loc,log=log, alt=vis) # alt. visibility
            
                print(" UPDATED ")
                grb.update_visibility(vis) 
                grb.show_visibility(loc=loc,log=log) # New visibility    
