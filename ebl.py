        ...
        for att, tag in zip(eblmodels, tags):
            ...
            for i, z in enumerate(redshifts):
                ...
                if isinstance(att, EBLAbsorptionNormSpectralModel):
                    attenuation = att.evaluate(energies,redshift=z, alpha_norm=1)
                else:
                    attenuation = att(energies, z)
                axa.plot(energies, attenuation, label = tag+" z="+str(round(z,2)),
...
if __name__ == "__main__":
    ...
    gilmore_ln   = EBL_from_file("data/ebl/others/"+"ebl_gilmore12.dat",debug=True)
    ...
    os.environ['GAMMAPY_DATA'] = str(Path(Path(__file__).absolute().parent,"data"))
