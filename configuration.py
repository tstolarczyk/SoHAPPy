...
class Configuration(object):
    ...
    ###------------------------------------------------------------------------
    @classmethod
    def build(cls, argv, conf_file = def_conf, debug=False):
        """
        ...
        for opt, arg in opts:
            if opt in ("-c","--config"):
                if Path(arg).is_file():
                    ...
                    sys.exit(f"{__name__:}.py: "\
                             f"{arg:} configuration file does not exist")
    ...
    def write(self, out_name = None):
        ...
        """
        if out_name == None: out_name = Path("config_backup.yaml")
