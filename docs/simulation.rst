...
The counts are passed to a :class:`Analysis` instance to compute the running
significances along slices, using the Li&Ma formula. From these running
...
* The normal use is to generated many trials -of the order of 100- fluctuating
  the signal and background at each time slice of the :class:`Slot`, in order
  to address, in particular, objects for which the 5 sigma detection is not
  granted depending on the count fluctuations. This use is for studying the
...
The functions of the :class:`Analysis` class handle the list over time of the
on and off-counts provided by the :class:`MonteCarlo class`. It
