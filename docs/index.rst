...
Two analysis methods are implemented :
    ...
    - a 3D likelhood meeting as implemented in Gammapy, based on the full enclosure off axis IRF (the one that are available if fits for the whole consortium). The siginificance is computed as the difference inthe log-likelihood of a fit with signal and background and a fit with background only.
...
.. automodapi:: plot_utils

.. automodapi:: montecarlo

.. automodapi:: grb

.. automodapi:: cta_irf_onaxis

.. automodapi:: cta_grb_observation
