...
Time slots are defined through the :class:`Slot` class in
:obj:`timeslot.py` and contain a collection of time slices handled by
the :class:`Slice` class in :obj:`obs_slice.py`. The time slots
...
:obj:`timeslot.py`. It is the key element of the `SoHAPPy` analysis.
...
At that stage the tie slot can be passed to the :class:`mcsim.MonteCarlo`
object.
...
Time slices
-----------
A time slice, handled by the :class:`obs_slice.Slice` in :obj:`obs_slice.py`,
...
by the user in the configuration file). The associated IRFs are obtained
according to the altitude (zenith) and slice duration at the beginning of the
slice where the flux is expected to be higher. The flux is the flux from the
GRB which is the closest in time to the observation time.
